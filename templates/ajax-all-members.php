		...
		while($row = mysql_fetch_array($result)){
			...
			*/
			...
			if (is_null($consent)){ // null = no connection to this person
				$addbutton = "
				<button alt='$user_id' title='add $first_name $last_name to your connections' class='add-button'>Add $first_name as Connection</button>";
			}elseif($consent>0){    // greater than zero means already a friend
				$addbutton = "<button class='added-button'>Already a Connection</button><h4>Already a Connection!</h4>";
			}else{ 					// 0 = friend request sent and waiting for it
				$addbutton = "<button class='added-button'>Added Connection</button><h4>Connection Request Sent</h4>";
			}
